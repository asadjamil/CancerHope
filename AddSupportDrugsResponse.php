<?php
	...
	if($page == 'add'){
		...
		/*$query1 = "SELECT tblzluotherdrugs.Name,tblzluotherdrugs.id AS OtherDrugID
				 	FROM tblzluotherdrugs WHERE tblzluotherdrugs.Name='$type'";
		$result1 = mysqli_query($conn,$query1);
		$row1 = mysqli_fetch_assoc($result1);
		$OtherDrugID = $row1["OtherDrugID"];*/
		...
		$query3 = "INSERT INTO tblotherdrugs(PatientID,TreatmentTypeID,TreatmentDate,SideEffectID) VALUES('$pid','$typeId','$date','$SideEffectID')";
		$result3 = mysqli_query($conn,$query3);

		

		/*$query3 = "INSERT INTO tblotherdrugssideeffects(PatientID,SideEffectId) VALUES('$pid','$SideEffectID')";
		$result3 = mysqli_query($conn,$query3);
		*/
	}
	...
	elseif ($page == 'del') {
		$id = $_POST['id'];
		$query4 = "DELETE FROM tblotherdrugs WHERE id = '$id' ";
		...
		$pid = $_POST['pid'];
		$query = "SELECT * FROM tblotherdrugs WHERE PatientID='$pid' ";
		...
		for ($i=0; $i<$rows; $i++)
		{
			...
			$query1 = "SELECT tblzluotherdrugs.id,tblzluotherdrugs.Name AS OtherDrugName FROM tblzluotherdrugs
					WHERE tblzluotherdrugs.id='$TreatmentTypeID'";
					$result1 = mysqli_query($conn,$query1);
					$row1 = mysqli_fetch_assoc($result1);
					$OtherDrugName = $row1["OtherDrugName"];
		?>
		<tr>
			
			<td><?php echo $TreatmentDate ?></td>
			<td><?php echo $OtherDrugName ?></td>
			...
			?>
				<td>
					...
					<button type="submit" onclick="deleteDataOtherSupportDrugs(<?php echo $id ?>,<?php echo $pid ?>)" class="btn btn-danger">Delete</button>
				<td>
			<?php
				}
			?>		
