<?php
	...
	if($page == 'add'){
		...
		$query3 = "INSERT INTO tblcomplementary(PatientID,TreatmentTypeID,TreatmentDate,SideEffectID) 
	...
	elseif ($page == 'del') {
		$id = $_POST['id'];
		$query4 = "DELETE FROM tblcomplementary WHERE id = '$id' ";
		...
		$pid = $_POST['pid'];
		$query = "SELECT * FROM tblcomplementary WHERE PatientID='$pid' ";
		...
		for ($i=0; $i<$rows; $i++)
		{
			...
			$query1 = "SELECT tblzlucomplementary.id,tblzlucomplementary.Name AS ComplementaryName FROM tblzlucomplementary
					WHERE tblzlucomplementary.id='$TreatmentTypeID'";
					$result1 = mysqli_query($conn,$query1);
					$row1 = mysqli_fetch_assoc($result1);
					$ComplementaryName = $row1["ComplementaryName"];
		?>
		<tr>
			
			<td><?php echo $TreatmentDate ?></td>
			<td><?php echo $ComplementaryName ?></td>
			...
			?>
				<td>
					...
					<button type="submit" onclick="deleteComplemantaryProcedures(<?php echo $id ?>,<?php echo $pid ?>)" class="btn btn-danger">Delete</button>
				<td>	
			<?php
				}
			?>		
